...
<body>
	...
	class Typo3Commands {
		...
		function setUseLoginSession( $useLoginSession ) {
		...
		function setTypo3Version( $typo_version ) {
		...
		function setHttpSourceDomain( $http_source_domain ) {
		...
		function setHttpSourcePackage( $http_source_package ) {
		...
		function setLocalTarDirectory( $local_tar_directory ) {
		...
		function setTypo3ConfigurationFile( $typo3_configuration_file ) {
		...
		function setMysqlImportDumpFile( $mysql_import_dump_file ) {
		...
		function setHtpasswdLine( $htpasswd_line ) {
		...
		function setConvertLatin1ToUtf8Configuration( $convertLatin1ToUtf8Configuration ) {
		...
		function checkConfigurationVariables() {
		...
		function do_exec_tar_extract( $tarFile, $parameter = "-xzvf" ) {
			if ( !is_file( $tarFile ) ) {
		...
		function do_exec_make_tar_gz( $tarFile, $source, $parameter = "-cpvlzf" ) {
			if ( is_file( $tarFile ) ) {
		...
		function do_rmdir_recursive( $path, $topLevel = false ) {
		...
		function do_error_echo( $content ) {
		...
		function do_convert_latin1_on_utf8_to_utf8() {
			// load the typo3Configuration
			$this->loadTypo3Configuration();
		...
		function do_convert_all_tables_to_utf8_general_ci() {
			// load the typo3Configuration
			$this->loadTypo3Configuration();
		...
		function loadTypo3Configuration() {
		...
		function checkDatabasePartInTypo3Configuration() {
		...
		function buildMysqlImportCommand() {
...
SCRIPT;
		...
		function showPasswordForm( $message = "", $errorMessageStyle = "error" ) {
			$html = '';
